<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="pId" width="50" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column label="操作" prop="operation" align="center" width="100" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button type="primary" size="small" :disabled="judge(row)" @click="handleUpdate(row)">
            录入数据
          </el-button>
        </template>
      </el-table-column>
    ...
    <!--    操作按钮弹窗-->
    <div>
      <my-dialog :title="dialogTitle" :form-list="formList" :dialog-visible.sync="dialogVisible" :form="form" :update-method="update" />
    </div>
    <!--    <el-dialog title="录入数据" :visible.sync="dialogFormVisible">-->
    ...
    <!--          {{ formList.unitPrice }}-->
    <!--        </el-form-item>-->
    <!--        <el-form-item label="实际数量" prop="remark">-->
    <!--          <el-input v-model="form.realNum" type="input" :rows="2" placeholder="请输入实际数量" oninput="value=value.replace(/[^\d]/g,'')" />-->
    <!--        </el-form-item>-->
...
import { changeState, checkOrder, updateOrder } from '@/api/check'
...
export default {
  ...
  data() {
    return {
      ...
      formList: [
        ...
        { label: '实际数量', prop: 'realNum', type: 'input', inputType: 'input', rows: 2, placeholder: '请输入实际数量' },
  ...
  methods: {
      ...
      checkOrder(req).then(response => {
        if (response.code === 200) {
          response.data.checkOrder.forEach(item => {
            ...
            if (item.realNum) {
              data.realNum = item.realNum
              data.diffNum = item.diffNum
              data.diffPrice = item.diffPrice
            } else {
              data.realNum = '-'
              data.diffNum = '-'
              data.diffPrice = '-'
    ...
    handleUpdate(row) {
      ...
      this.updateRequest.orderId = row.id
      ...
      // this.$refs.myDialog.formList = this.formList
    },
    // handleUpdate(row) {
    //   this.form = {
    //     realNum: '',
    ...
    //   this.updateRequest.orderId = row.id
    //   this.formList.pId = row.positionId
    //   this.formList.goodsName = row.goodsName
    //   this.formList.recordNum = row.recordNum
    //   this.formList.unitPrice = row.unitPrice
    // },
    update() {
      // if (this.form.realNum.)
      this.dialogVisible = false
      this.updateRequest.realNum = this.form.realNum
      updateOrder(this.updateRequest).then(response => {
        if (response.code === 200) {
          const changeReq = {}
          changeReq.state = 2
          changeReq.orderId = this.updateRequest.orderId
          changeReq.userId = this.$store.getters.userId
          changeReq.remark = this.form.remark
          changeState(changeReq).then(resp => {
            if (resp.code === 200) {
              this.$message({
                showClose: true,
                type: 'success',
    ...
    judge(row) {
      // 启用按钮
      if (row.state === '数据未录入' || row.state === '审核未通过') {
        return false
      } else {
        return true
      }
