<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="入库单ID" prop="orderId" width="180" align="center">
...
export default {
  ...
  methods: {
    ...
    handleAbnormal(row) {
      ...
      this.changeList.state = 2
      this.changeList.userId = this.$store.getters.userId
      this.changeList.remark = '验收不合格'
    },
    update() {
      if (this.form.remark !== '') {
      ...
      changeState(this.changeList).then(() => {
        ...
        this.$notify({
          title: 'Success',
          message: 'Pre inspection successfully',
          type: 'success',
          duration: 2000
        })
      })
    ...
    handelNormal(row) {
      this.dialogStatus = '验收合格'
      this.dialogFormVisible = true
      this.changeList = {}
      this.changeList.orderId = row.orderId
      this.changeList.state = 3
      this.changeList.userId = this.$store.getters.userId
      this.changeList.remark = '验收合格'
    },
    getSpanArr() {
