<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="创建时间" min-width="230" align="center">
        <template slot-scope="{row}">
          <span>{{ row.time }}</span>
        </template>
      </el-table-column>
      <el-table-column label="出库单描述" width="200" align="center">
        <template slot-scope="{row}">
          <span>{{ row.desc }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap['approve']" :visible.sync="approveFormVisible">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button v-if="form.state===1" type="primary" @click="handlePickUp">
          拣货
        </el-button>
        <el-button v-if="form.state===2" type="primary" @click="handleReview">
          复核
        </el-button>
        <el-button v-if="form.state===3" type="primary" @click="handlePack">
          打包
        </el-button>
...
export default {
  ...
  methods: {
    getList() {
      ...
      outOrder({ 'type': '1,2,3' }).then(response => {
    ...
    handlePickUp() {
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          approve.state = 16
          approve.id = parseInt(this.form.id)
          approve.remark = this.form.remark
          approve.operator = parseInt(this.$store.getters.userId)
          changeState(approve).then(() => {
    ...
    handleReview() {
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          review.state = 17
          review.id = parseInt(this.form.id)
          review.remark = this.form.remark
          review.operator = parseInt(this.$store.getters.userId)
          changeState(review).then(() => {
            this.approveFormVisible = false
            this.$notify({
              title: 'Success',
    ...
    handlePack() {
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          pack.state = 18
          pack.id = parseInt(this.form.id)
          pack.remark = this.form.remark
          pack.operator = parseInt(this.$store.getters.userId)
          changeState(pack).then(() => {
            this.approveFormVisible = false
            this.$notify({
              title: 'Success',
              message: 'Pack Successfully',
              type: 'success',
              duration: 2000
            })
          })
